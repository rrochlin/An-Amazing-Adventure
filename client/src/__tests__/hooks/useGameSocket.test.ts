...
describe("useGameSocket", () => {
  ...
  it("dispatches narrative_chunk frame to store", async () => {
    ...
    act(() => {
      lastWs?.onmessage?.({
        data: JSON.stringify({ type: "narrative_chunk", payload: { content: "The goblin attacks!" } }),
      });
    });
    expect(useGameStore.getState().streamingMessage).toBe("The goblin attacks!");
  ...
  it("finalizes streaming on narrative_end", async () => {
    ...
    await flushPromises();
    act(() => {
      lastWs?.onmessage?.({ data: JSON.stringify({ type: "narrative_chunk", payload: { content: "Hello" } }) });
      lastWs?.onmessage?.({ data: JSON.stringify({ type: "narrative_end" }) });
    });
  ...
  it("sets wsError on error frame", async () => {
    ...
    act(() => {
      lastWs?.onmessage?.({
        data: JSON.stringify({ type: "error", payload: { message: "Something went wrong" } }),
      });
    });
  ...
  it("sendChat sends JSON over WS when open", async () => {
    const { result } = renderHook(() => useGameSocket({ sessionId: "sess-1" }));
    await flushPromises();
    act(() => { result.current.sendChat("Go north"); });
  ...
  it("sendAction sends game_action JSON over WS", async () => {
    const { result } = renderHook(() => useGameSocket({ sessionId: "sess-1" }));
    await flushPromises();
    act(() => { result.current.sendAction("move", "north"); });
    expect(lastWs?.send).toHaveBeenCalledWith(
      JSON.stringify({ action: "game_action", sub_action: "move", payload: "north" })
    );
  });
  ...
  it("closes WebSocket on unmount", async () => {
    const { unmount } = renderHook(() => useGameSocket({ sessionId: "sess-1" }));
    await flushPromises();
    unmount();
