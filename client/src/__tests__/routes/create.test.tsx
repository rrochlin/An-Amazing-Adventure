...
const { mockCreateGame, mockJoinCharacter } = vi.hoisted(() => ({
   mockCreateGame: vi.fn().mockResolvedValue({ session_id: "abc-123", ready: false, preview_mode: false }),
   mockJoinCharacter: vi.fn().mockResolvedValue({ session_id: "existing-session" }),
}));
...
describe("CreateRoute wizard — new game mode", () => {
   ...
   it("Race step: can select a race without subraces and advance", async () => {
      ...
      expect(screen.getByText(/Your class defines your combat style/i)).toBeInTheDocument();
   });
   ...
   it("Ability scores: Next disabled until all 6 assigned", async () => {
      ...
      expect(screen.getByText(/Assign the standard array/i)).toBeInTheDocument();
      expect(screen.getByRole("button", { name: /next/i })).toBeDisabled();
   ...
   it("Skills: limited to class skill count", async () => {
      ...
      expect(screen.getByText(/Choose 2 skill proficiencies from the Barbarian list/i)).toBeInTheDocument();
      // Select 2 skills
   ...
   it("stepper shows all 7 steps in create mode", () => {
      renderWizard();
      ["Name", "Race", "Class", "Ability Scores", "Skills", "Adventure", "Review"].forEach((label) => {
...
describe("CreateRoute wizard — join game mode", () => {
   ...
   it("shows info alert about joining", () => {
      renderWizard();
      expect(screen.getByText(/joining an existing adventure/i)).toBeInTheDocument();
   });

   it("stepper shows only 6 steps in join mode (no Adventure step)", () => {
      renderWizard();
      ["Name", "Race", "Class", "Ability Scores", "Skills", "Review"].forEach((label) => {
         expect(screen.getByText(label)).toBeInTheDocument();
      });
   ...
   it("submit in join mode calls JoinCharacter, not CreateGame", async () => {
      ...
      for (let i = 0; i < abilitySelects.length; i++) {
         ...
         const listbox = screen.getByRole("listbox");
         const options = within(listbox).getAllByRole("option");
         const targetOption = options.find((o) => o.textContent === String(values[i]));
         if (targetOption) await user.click(targetOption);
      }
      await clickNext(user); // → Skills
      await user.click(screen.getByText("Athletics (STR)"));
      ...
      expect(mockJoinCharacter).toHaveBeenCalledWith(
         ...
         expect.objectContaining({
            ...
            race_id: "human",
            class_id: "fighter",
            selected_skills: expect.arrayContaining(["athletics", "perception"]),
         }),
      );
