...
export const RoomMap = ({ gameState }: { gameState: GameState }) => {
  ...
  const roomPositions = calculateRoomPositions(gameState.connected_rooms);

  const handleRoomHover = (roomId: string, pos: { x: number; y: number }) => {
    setHoveredRoom(roomId);
    const isAdjacent = gameState.current_room.connections.includes(
      gameState.connected_rooms[roomId],
    );
  ...
  return (
    <Stage width={stageWidth} height={stageHeight} draggable>
      <Layer>
        {/* Draw connections */}
        {Object.entries(gameState.connected_rooms).map(([roomId, room]) =>
          room.connections.map((conn) => {
            const start = roomPositions[roomId];
            const end = roomPositions[conn.id];
            ...
            const isCurrentRoomConnection =
              ...
              conn.id === gameState.current_room.id;
