...
export const RoomMap = ({ gameState }: { gameState: GameState }) => {
  ...
  const roomPositions = useMemo(() => {
    return calculateRoomPositions(gameState);
  }, [gameState]);

  const handleRoomHover = (roomId: string, pos: { x: number; y: number }) => {
    setHoveredRoom(roomId);
    const isAdjacent = gameState.current_room.connections.includes(roomId);
  ...
  useEffect(() => {
    const valueInPixels = (20 / 100) * viewportWidth;
    setCalculatedValue(valueInPixels);
  }, [viewportWidth]);

  return (
    <Stage width={calculatedValue} height={stageHeight}>
      <Layer>
        {/* Draw connections */}
        {gameState.rooms &&
          Object.keys(gameState.rooms).map(
            (roomId) =>
              gameState.rooms &&
              gameState.rooms[roomId].connections.map((conn) => {
                const start = roomPositions[roomId];
                const end = roomPositions[conn];
                ...
                const isCurrentRoomConnection =
                  ...
                  conn === gameState.current_room.id;
        ...
        {/* Draw rooms */}
        {gameState.rooms &&
          Object.keys(gameState.rooms).map((roomId) => {
            ...
            return (
              <Fragment key={roomId}>
                <Circle
                  ...
                  shadowColor="black"
                  shadowBlur={
                    isCurrentRoom ? 15 : isConnectedToCurrent ? 12 : 10
                  }
                  shadowOpacity={
        ...
        {gameState.connected_rooms &&
          gameState.connected_rooms.map((roomId) => {
            const pos = roomPositions[roomId];
            if (!pos) return null;
            const room = gameState.rooms
              ? gameState.rooms[roomId]
            ...
            const isAdjacent =
              room.connections.includes(gameState.current_room.id) ||
              roomId === gameState.current_room.id;
