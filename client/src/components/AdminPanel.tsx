...
export function AdminPanel() {
   ...
   const [edits, setEdits] = useState<
      Record<string, Partial<UpdateUserRequest>>
   >({});
   ...
   async function handleSave(user: AdminUserView) {
      ...
      try {
         ...
         setError(
            `Save failed for ${user.email || user.user_id}: ${String(e)}`,
         );
      } finally {
      ...
      <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
         ...
         {stats && (
            <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
               ...
               <Chip
                  label={`${stats.approved_users} approved`}
                  color="success"
               />
               <Chip
                  label={`${stats.restricted_users} restricted`}
                  color="error"
               />
               <Chip
                              ...
                              <Select
                                 ...
                                 onChange={(ev: SelectChangeEvent) =>
                                    patchEdit(user.user_id, {
                                       role: ev.target
                                          .value as UpdateUserRequest['role'],
                                    })
                                 }
                              >
